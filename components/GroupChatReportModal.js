...
export default function GroupChatReportModal({
  ...
  const canDelete = isOwnMessage || userRole === "admin" || userRole === "super_admin";
  ...
  const handleReportSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("group_chat_reports").insert({
      ...
      // Log the report
      await logEvent({
        event_type: "GROUP_MESSAGE_REPORTED",
        user_id: user.id,
        target_id: message.id,
        target_table: "group_messages",
        details: { reason: selectedReason, group_id: groupId },
      });

      Toast.show({
        type: "success",
        text1: "Message reported",
        text2: "Our team will review this shortly",
      });

      // Call parent callback if exists
      if (onReport) {
        await onReport(message.id, message.message, selectedReason);
      }
