...
export default function Login() {
  ...
  const { bgCard, bgInput, borderInput, textPrimary, placeholderColor } = useAppTheme();

  const isValidEmail = (emailStr) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailStr);

  const showToast = (type, title, message) => Toast.show({ type, text1: title, text2: message });

  const handlePostAuthRouting = async (userId) => {
    const { data: admin } = await supabase
      .from("admins").select("id").eq("id", userId).single();
    ...
    const { data: teacher, error: teacherError } = await supabase
      .from("teachers").select("verified, is_disabled, disabled_reason").eq("id", userId).single();
    ...
    if (teacher.is_disabled) {
      ...
      logEvent({ event_type: "LOGIN_BLOCKED_DISABLED", user_id: userId, details: { reason: teacher.disabled_reason } });
      return;
    }
    ...
    const { data: membership } = await supabase
      .from("memberships").select("active").eq("id", userId).single();

    showToast("success", "Login Successful", "Welcome back!");
    setTimeout(() => router.replace(membership?.active ? "/(tabs)" : "/membership"), 1500);
  };
  ...
  const handleOAuthSignIn = async (provider) => {
    ...
    try {
      ...
      const result = await WebBrowser.openAuthSessionAsync(data.url, 'teacherhub://auth/callback');
      ...
      if (result.type === 'success') {
        ...
        if (accessToken) {
          ...
          logEvent({ event_type: `LOGIN_SUCCESS_${provider.toUpperCase()}`, user_id: sessionData.user.id });
  ...
  const handleLogin = async () => {
    ...
    if (error || !data.session) {
      setLoading(false);
      logEvent({ event_type: "LOGIN_FAILED", details: { email, error: error?.message } });
    ...
    if (aalData.nextLevel === "aal2" && aalData.nextLevel !== aalData.currentLevel) {
      ...
      if (totpFactor) {
        ...
        setLoading(false);
        router.replace({ pathname: "/mfa-challenge", params: { factorId: totpFactor.id, challengeId: challengeData.id } });
        return;
      }
    }

    logEvent({ event_type: "LOGIN_SUCCESS", user_id: data.user.id, details: { email } });
    await handlePostAuthRouting(data.user.id);
    setLoading(false);
  };

  const handleForgotPassword = async () => {
    if (!forgotEmail) { showToast("error", "Missing Email", "Please enter your email"); return; }
    if (!isValidEmail(forgotEmail)) { showToast("error", "Invalid Email", "Please enter a valid email"); return; }
    ...
    <ScreenWrapper>
      ...
      <View className="flex-1 justify-center items-center">
        <View className={`w-full max-w-md ${bgCard} p-6 rounded-xl shadow-lg`}>
          ...
          <TouchableOpacity onPress={() => setShowForgotModal(true)} disabled={loading}>
          ...
          >
            ...
            <Text className="text-gray-800 font-semibold text-center flex-1">Continue with Google</Text>
          </TouchableOpacity>
      ...
      <Modal visible={showDisabledModal} animationType="slide" transparent onRequestClose={() => setShowDisabledModal(false)}>
        <View className="flex-1 justify-center items-center bg-black/60">
          <View className={`w-full max-w-md ${bgCard} p-6 rounded-2xl shadow-lg`}>
            <View className="items-center mb-4">
              ...
              <Text className="text-2xl font-bold text-center text-red-500">Account Disabled</Text>
            </View>
            <Text className={`text-center ${textPrimary} mb-4 text-base leading-6`}>{disabledReason}</Text>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                router.push({ pathname: "/contact", params: { subject: "Account Reactivation Request", from: "disabled-account" } });
              }}
            >
              <Text className="text-white text-center font-semibold">Contact Admin</Text>
            </TouchableOpacity>
            <TouchableOpacity
              className={`${bgInput} p-4 rounded-xl border ${borderInput}`}
              onPress={() => { setShowDisabledModal(false); supabase.auth.signOut(); }}
            >
              <Text className="text-center text-cyan-400 font-semibold">OK</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
